...
* @version $Id: content_flowplayer.php 39730 2014-04-07 21:05:30Z weinert $
...
class content_flowplayer extends base_content {
  ...
  */
  function getParsedData() {
    ...
    } elseif (PapayaFilterFactory::isUrl($this->data['flashfile'], TRUE) &&
              (strtolower(substr($this->data['flashfile'], -4)) == '.swf')) {
      $fileName = $this->data['flashfile'];
    } else {
      $fileName = $this->papaya()->options->get(
        'PAPAYA_PATH_WEB', '/'
      ).'papaya-script/FlowPlayer.swf';
    }
    if ($movie = $mediaDB->getFile($this->data['flashmovie'])) {
      ...
      $flashVars = array(
        ...
        'progressBarColor2' => $this->data['progressBarColor2'],
